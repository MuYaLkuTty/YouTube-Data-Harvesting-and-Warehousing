...
def api_connect():
    Api_Id = 'AIzaSyDPFAH0iJL7MsNyQg0CoQrSWPOe61T9u6Y'
...
def get_channel_info(channel_id):
    ...
    reaponse1 = request.execute()

    for i in range(o,len(response1["items"])):
        data = dict(
            channel_Name = response1["items"][i]["snippet"]["title"],
            channel_Is = esponse1["items"][i]["id"],
            Subcription_Count = response1["items"][i]["statistics"]["subscriberCount"],
...
#MongoDB Connection
client = pymongo.MongoClient("mongodb+srv://gowrishankar:1997@cluster0.yaryscs.mongodb.net/?retryWrites=true&w=majority")
